...
class MyOptimizer1(Optimizer):
    def initialize(self):

        # Name your algorithm
        self.solver_name = 'Steepest Descent'
    ...
    def solve(self):
        ...
        while (opt > opt_tol and itr < max_itr):
            ...
            # >>>>>>>>>>>>>>>>>>>>>
            p_k = - g_k
            alpha = self.backls(rho, c1, alphab, x_k, p_k)
    ...
    def backls(self, rho, c1, alphab, xk, pk):
        ...
        obj = self.obj
        grad = self.grad
        while obj(xk + alpha * pk) >= obj(xk) + c1 * alpha * np.dot(np.transpose(grad(xk)), pk):
            alpha = rho * alpha
